...
def signals(
    symbol: Optional[str] = typer.Option(
        None, "--symbol", "-s", help="Currency pair symbol (e.g., EURUSD)"
    ),
    active_only: bool = typer.Option(
        False, "--active", "-a", help="Show only active signals"
    ),
    ...
    try:
        ...
        for signal in signals:
            ...
            typer.echo(
                f"  {status} {signal.get('symbol', 'N/A')} - {signal.get('signal_type', 'N/A')}"
            )
        ...
        for signal in cond_signals:
            typer.echo(
                f"  📈 {signal.get('condition', 'N/A')} -> {signal.get('action', 'N/A')}"
            )
...
def positions(
    symbol: Optional[str] = typer.Option(
        None, "--symbol", "-s", help="Filter by currency pair"
    ),
    close_all: bool = typer.Option(
        False, "--close-all", help="Close all positions (requires confirmation)"
    ),
):
    ...
    try:
        ...
        if symbol:
            filtered_positions = [
                p for p in positions if p.get("symbol") == symbol.upper()
            ]
        ...
        for pos in filtered_positions:
            ...
            typer.echo(
                f"{pnl_symbol} {pos.get('symbol', 'N/A')} - {pos.get('type', 'N/A')}"
            )
...
def start():
    ...
    try:
        ...
        if result:
            typer.secho(
                "✅ Automated trading started successfully", fg=typer.colors.GREEN
            )
        else:
...
def calendar():
    ...
    try:
        ...
        for event in events:
            ...
            impact_symbol = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(
                event.get("impact", "low").lower(), "⚪"
            )

            typer.echo(
                f"{impact_symbol} {event.get('time', 'N/A')} - {event.get('currency', 'N/A')}"
            )
