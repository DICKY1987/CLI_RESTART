...
def health():
    ...
    try:
        ...
        if overall == "healthy":
            ...
            typer.secho(
                f"⚠️  Overall Status: {overall.upper()}",
                fg=typer.colors.YELLOW,
                bold=True,
            )
        else:
            typer.secho(
                f"❌ Overall Status: {overall.upper()}", fg=typer.colors.RED, bold=True
            )
            ...
            for category, category_tiles in tiles.items():
                ...
                for tile_name, tile_data in category_tiles.items():
                    ...
                    if status == "healthy":
                        ...
                        typer.secho(
                            f"  ⚠️  {tile_name}: {value}", fg=typer.colors.YELLOW
                        )
                    else:
...
def config(
    ...
    key: Optional[str] = typer.Option(
        None, "--key", "-k", help="Configuration key to view/set"
    ),
    value: Optional[str] = typer.Option(
        None, "--value", "-v", help="Value to set for the key"
    ),
    ...
    try:
        ...
        if reset:
            ...
            if confirm:
                ...
                typer.secho(
                    "🔄 Configuration reset (restart required)", fg=typer.colors.GREEN
                )
            return
        ...
        if key and value:
            ...
            typer.secho(
                f"✅ Configuration updated: {key} = {value}", fg=typer.colors.GREEN
            )
            return
        ...
        if key:
            ...
            try:
                ...
                typer.secho(
                    f"❌ Configuration key '{key}' not found", fg=typer.colors.RED
                )
            return
...
def logs(
    tail: int = typer.Option(
        50, "--tail", "-n", help="Number of recent log entries to show"
    ),
    follow: bool = typer.Option(
        False, "--follow", "-f", help="Follow log file (tail -f behavior)"
    ),
    level: Optional[str] = typer.Option(
        None, "--level", help="Filter by log level (INFO, WARNING, ERROR)"
    ),
):
    """View system logs"""
    try:
        ...
        # Find log files
        log_patterns = ["*.log", "logs/*.log", "production_validation_*.log"]
...
def backup():
    ...
    try:
        ...
        typer.secho(
            f"✅ Backup created: {backup_name} ({backup_size:.1f} MB)",
            fg=typer.colors.GREEN,
            bold=True,
        )
