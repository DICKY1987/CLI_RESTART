...
def status():
    ...
    try:
        from eafix.guardian.agents import MarketAgent, RiskAgent, SystemAgent
        from eafix.guardian.constraints import ConstraintRepository

        typer.secho(
            "🛡️  Guardian Protection System Status", fg=typer.colors.BLUE, bold=True
        )
        typer.echo("-" * 50)
        ...
        for constraint in constraints[:10]:  # Show first 10
            ...
            typer.echo(
                f"  {status_icon} {constraint.get('name', 'N/A')} - {constraint.get('type', 'N/A')}"
            )
        ...
        for name, agent in agents:
            try:
                ...
                typer.echo(
                    f"  {health_icon} {name}: {agent_status.get('status', 'Unknown')}"
                )
...
def constraints(
    enable: Optional[str] = typer.Option(
        None, "--enable", help="Enable constraint by name"
    ),
    disable: Optional[str] = typer.Option(
        None, "--disable", help="Disable constraint by name"
    ),
    ...
    try:
        ...
        if enable:
            ...
            if success:
                ...
                typer.secho(
                    f"❌ Failed to enable constraint '{enable}'", fg=typer.colors.RED
                )
            return
        ...
        if disable:
            ...
            if success:
                typer.secho(
                    f"⚠️  Constraint '{disable}' disabled", fg=typer.colors.YELLOW
                )
            else:
                typer.secho(
                    f"❌ Failed to disable constraint '{disable}'", fg=typer.colors.RED
                )
            return

        # List constraints
        constraints = (
            repo.get_all_constraints() if list_all else repo.get_active_constraints()
        )
...
def gates():
    ...
    try:
        ...
        if all_open:
            typer.secho(
                "✅ All gates are OPEN - Trading allowed",
                fg=typer.colors.GREEN,
                bold=True,
            )
        else:
            typer.secho(
                "❌ Some gates are CLOSED - Trading restricted",
                fg=typer.colors.RED,
                bold=True,
            )
...
def emergency():
    ...
    try:
        ...
        if action == "1":
            ...
            typer.secho(
                "⏸️  Trading decisions PAUSED", fg=typer.colors.YELLOW, bold=True
            )
            ...
            if confirm:
                ...
                typer.secho(
                    f"📊 {result.get('closed_positions', 0)} positions closed",
