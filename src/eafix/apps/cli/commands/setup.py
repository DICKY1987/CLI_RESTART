...
def install(
    ...
    try:
        ...
        req_files = ["mt4_dde_interface/requirements.txt", "requirements.txt"]
        ...
        for req_file in found_requirements:
            ...
            if result.returncode == 0:
                ...
                typer.secho(
                    f"❌ Failed to install from {req_file}", fg=typer.colors.RED
                )
        ...
        if dev:
            ...
            dev_packages = ["pytest", "pytest-asyncio", "typer[all]", "rich", "psutil"]
...
def init(
    ...
    template: str = typer.Option(
        "basic", "--template", "-t", help="Template type (basic, full, minimal)"
    ),
):
    ...
    try:
        ...
        typer.secho(
            f"🚀 Initializing EAFIX project in {target_dir}",
        ...
        if template == "minimal":
            directories = ["src/eafix/signals", "tests", "logs"]
            files = {
                "settings.json": """{
  "signals": {
    "FridayMove": {
  ...
  "dde": {
    "poll_ms": 100
        ...
        for file_path, content in files.items():
            ...
            if not full_path.exists() or typer.confirm(
                f"File {file_path} exists. Overwrite?"
            ):
        ...
        typer.secho(
            f"\\n✅ Project initialized with {template} template!",
...
def test_install():
    ...
    try:
        typer.secho(
            "🧪 Testing EAFIX CLI Installation", fg=typer.colors.BLUE, bold=True
        )
        typer.echo("-" * 40)
        ...
        for module, description in test_imports:
            try:
                ...
                typer.secho(
                    f"  ❌ {module}: {description} - MISSING", fg=typer.colors.RED
                )
        ...
        for file_path, description in required_paths:
            ...
            if path.exists():
                ...
                typer.secho(
                    f"  ⚠️  {file_path}: {description} - MISSING", fg=typer.colors.YELLOW
                )
        ...
        if config_file.exists():
            try:
                ...
                with open(config_file) as f:
                    config = json.load(f)
                typer.secho(
                    "  ✅ Configuration file is valid JSON", fg=typer.colors.GREEN
                )
                ...
                for section in required_sections:
                    if section in config:
                        typer.secho(
                            f"  ✅ Configuration has {section} section",
                            fg=typer.colors.GREEN,
                        )
                    else:
                        typer.secho(
                            f"  ⚠️  Configuration missing {section} section",
                            fg=typer.colors.YELLOW,
                        )

            except json.JSONDecodeError:
                typer.secho(
                    "  ❌ Configuration file has invalid JSON", fg=typer.colors.RED
                )
        else:
        ...
        for db_file in db_files:
            if Path(db_file).exists():
                typer.secho(
                    f"  ✅ Database file exists: {db_file}", fg=typer.colors.GREEN
                )
            else:
                typer.secho(
                    f"  ⚠️  Database file not found: {db_file}", fg=typer.colors.YELLOW
                )
...
@app.command()
def configure(
    mt4_path: Optional[str] = typer.Option(
        None, "--mt4-path", help="MetaTrader 4 installation path"
    ),
    database_path: Optional[str] = typer.Option(
        None, "--db-path", help="Database directory path"
    ),
    ...
    try:
        ...
        if database_path:
            ...
            typer.secho(
                f"✅ Database path set to: {database_path}", fg=typer.colors.GREEN
            )
    ...
    try:
        ...
        try:
            ...
            if free_gb > 1:
                ...
                typer.secho(
                    f"  ⚠️  {free_gb} GB free space (low)", fg=typer.colors.YELLOW
                )
        ...
        if not issues_found:
            ...
            typer.secho(
                f"⚠️  Found {len(issues_found)} issues:",
                fg=typer.colors.YELLOW,
                bold=True,
            )
