...
app.add_typer(
    trading_app,
    name="trade",
    help="[bold blue]Trading operations and signals[/bold blue]",
)
app.add_typer(
    guardian_app,
    name="guardian",
    help="[bold red]Guardian protection system[/bold red]",
)
app.add_typer(
    system_app,
    name="system",
    help="[bold green]System management and monitoring[/bold green]",
)
app.add_typer(
    analysis_app,
    name="analyze",
    help="[bold yellow]Market analysis and reporting[/bold yellow]",
)
app.add_typer(
    setup_app, name="setup", help="[bold cyan]Setup and configuration[/bold cyan]"
)
app.add_typer(
    apf_app,
    name="apf",
    help="[bold magenta]Atomic Process Framework operations[/bold magenta]",
)
...
def status():
    ...
    try:
        ...
        if status["overall_health"] == "healthy":
            typer.secho(
                f"✅ System Status: {status['overall_health'].upper()}",
                fg=typer.colors.GREEN,
            )
        elif status["overall_health"] == "warning":
            typer.secho(
                f"⚠️  System Status: {status['overall_health'].upper()}",
        ...
        typer.secho(
            "⚠️  Health checker not available. Run 'eafix setup install' to install dependencies.",
            fg=typer.colors.YELLOW,
        )
...
def launch():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "launch_trading_system.py"],
            cwd=Path.cwd(),
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0:
            typer.secho(
                "✅ Trading system launched successfully", fg=typer.colors.GREEN
            )
        else:
