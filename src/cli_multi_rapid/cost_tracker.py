...
class CostTracker:
            ...
            for _vendor, vdata in (vendors or {}).items():
                ...
                for model_name, mdata in (models or {}).items():
                    ...
                    if isinstance(mdata, dict):
                        ...
                        if isinstance(inp, (int, float)) and isinstance(
                            out, (int, float)
                        ):
    ...
    def allocate_budget(
        ...
        remaining_budget = (
            coordination_budget.total_budget - coordination_budget.emergency_reserve
        )
        ...
        for workflow in workflows:
            ...
            priority = (
                workflow.get("metadata", {}).get("coordination", {}).get("priority", 2)
            )
            priority_multiplier = coordination_budget.priority_multipliers.get(
                priority, 1.0
            )
    ...
    def get_coordination_summary(self, coordination_id: str) -> Dict[str, Any]:
        ...
        workflows = defaultdict(
            lambda: {
                ...
                "operations": [],
                "phases": defaultdict(
                    lambda: {"tokens": 0, "cost": 0.0, "operations": 0}
                ),
            }
        )
        ...
        try:
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            if usage.get("coordination_id") == coordination_id:
                                ...
                                workflows[workflow_id]["total_tokens"] += usage[
                                    "tokens_used"
                                ]
                                workflows[workflow_id]["total_cost"] += usage[
                                    "estimated_cost"
                                ]
            ...
            console.print(
                f"[yellow]Warning: Could not read coordination usage - {e}[/yellow]"
            )
    ...
    def get_workflow_cost_summary(
        ...
        try:
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            if usage.get("workflow_id") == workflow_id and (
                                coordination_id is None
                                or usage.get("coordination_id") == coordination_id
                            ):
                                ...
                                if phase_id:
                                    ...
                                    summary.phases[phase_id]["tokens"] += usage[
                                        "tokens_used"
                                    ]
                                    summary.phases[phase_id]["cost"] += usage[
                                        "estimated_cost"
                                    ]
            ...
            console.print(
                f"[yellow]Warning: Could not read workflow usage - {e}[/yellow]"
            )
    ...
    def check_coordination_budget(
        ...
        budget_status = {
            ...
            "available_budget": coordination_budget.total_budget
            - coordination_budget.emergency_reserve,
            "used_budget": summary["total_cost"],
            "remaining_budget": coordination_budget.total_budget
            - summary["total_cost"],
            "budget_utilization": summary["total_cost"]
            / coordination_budget.total_budget,
            "within_budget": summary["total_cost"] <= coordination_budget.total_budget,
            "emergency_triggered": summary["total_cost"]
            > (
                coordination_budget.total_budget - coordination_budget.emergency_reserve
            ),
            "workflows": {},
        ...
        for workflow_id, workflow_data in summary["workflows"].items():
            allocated = coordination_budget.workflow_allocations.get(
                workflow_id, coordination_budget.per_workflow_budget
            )
            workflow_budget_status = {
                ...
                "utilization": (
                    workflow_data["total_cost"] / allocated if allocated > 0 else 0
                ),
    ...
    def optimize_remaining_allocation(
        ...
        remaining_budget = (
            coordination_budget.total_budget - current_summary["total_cost"]
        )

        if remaining_budget <= coordination_budget.emergency_reserve:
            # Emergency mode: minimal allocation
            emergency_per_workflow = coordination_budget.emergency_reserve / max(
                len(remaining_workflows), 1
            )
            return {
                workflow_id: emergency_per_workflow
        ...
        return {
            workflow_id: min(
                per_workflow_allocation, coordination_budget.per_workflow_budget
            )
            for workflow_id in remaining_workflows
        }
