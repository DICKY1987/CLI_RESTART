...
class CertificateGeneratorAdapter(BaseAdapter):
    ...
    def execute(
        ...
        try:
            ...
            is_valid = self._validate_certificate_requirements(
                certificate_data, verification_results
            )
            ...
            if isinstance(stage_results, dict):
                if "success" in stage_results:
                    stage_summary["status"] = (
                        "passed" if stage_results["success"] else "failed"
                    )
                ...
                if "results" in stage_results and isinstance(
                    stage_results["results"], list
                ):
    ...
    def _calculate_integrity_hashes(
        ...
        try:
            ...
            verification_json = json.dumps(
                verification_results, sort_keys=True, separators=(",", ":")
            )
            hashes["verification_results"] = hashlib.sha256(
                verification_json.encode()
            ).hexdigest()

            # Hash individual components if present
            if "files_modified" in verification_results:
                files_json = json.dumps(
                    verification_results["files_modified"], sort_keys=True
                )
                hashes["modified_files"] = hashlib.sha256(
                    files_json.encode()
                ).hexdigest()

            if "test_results" in verification_results:
                tests_json = json.dumps(
                    verification_results["test_results"], sort_keys=True
                )
            ...
            if "security_scan" in verification_results:
                security_json = json.dumps(
                    verification_results["security_scan"], sort_keys=True
                )
                hashes["security_scan"] = hashlib.sha256(
                    security_json.encode()
                ).hexdigest()
        ...
        for stage_name, stage_data in verification_results.items():
            if isinstance(stage_data, dict):
                chain_entry = {
                    ...
                    "status": (
                        "passed" if stage_data.get("success", False) else "failed"
                    ),
                    "hash": hashlib.sha256(
                        json.dumps(stage_data, sort_keys=True).encode()
                    ).hexdigest()[:16],
                }
    ...
    def _assess_compliance(
        ...
        for check_name, description in required_checks.items():
            if check_name in verification_results:
                ...
                if isinstance(check_result, dict) and check_result.get(
                    "success", False
                ):
    ...
    def _generate_signatures(self, certificate_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cert_content = {
                k: v for k, v in certificate_data.items() if k != "signatures"
            }
            content_json = json.dumps(
                cert_content, sort_keys=True, separators=(",", ":")
            )
            ...
            system_signature = hashlib.sha256(
                f"{content_hash}:{system_key}".encode()
            ).hexdigest()
        ...
        try:
            ...
            for stage in critical_stages:
                if stage in verification_results:
                    ...
                    if isinstance(stage_result, dict) and not stage_result.get(
                        "success", False
                    ):
                        return False
